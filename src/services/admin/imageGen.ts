  ...
  try {
    ...
    for (const termRaw of wordArray) {
      ...
      if (existing) {
        ...
        const promptId = await sendPromptAPI(existing.positivePrompt);
      ...
      console.log(
        `🆕 Word "${term}", checking for prompt ID...: ${wordData.exampleSentence}`
      );
      const promptId = await sendPromptAPI(wordData.positivePrompt);
